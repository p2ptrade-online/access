...
export default function SwapPage() {
  const [selectedShift, setSelectedShift] = useState(swapOptions[0].title);
    ...
    <div className="page swap-page">
      ...
      <section className="swap-section">
        ...
        <div className="swap-list">
          {swapOptions.map((shift) => (
            <button
              key={shift.title}
              className={`swap-card${selectedShift === shift.title ? ' active' : ''}`}
              type="button"
              onClick={() => setSelectedShift(shift.title)}
            >
              <div>
          ...
          {teammates.map((mate) => (
            <div key={mate.name} className="teammate-card">
              ...
              <button
                className="ghost-btn"
                type="button"
                onClick={() => setStatusMessage(`Swap request sent to ${mate.name} for ${selectedShift}.`)}
              >
                Request
              </button>
            </div>
